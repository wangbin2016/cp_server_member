...
public class MemberController<T> {
	...
	public ResponseEntity<ResponseData<MemberInfoResponse>> regist(CheckLoginParam param) {
		...
		try {
			...
			if (token == null || account == null) {
				responset = new ResponseData<MemberInfoResponse>(MemberResponseStatus.NOT_LOGIN, memberInfo);
			} else {
	...
	public ResponseEntity<ResponseData<DataBody>> getResponseContent(ResponseData<DataBody> responseData) {
		ResponseEntity<ResponseData<DataBody>> response = new ResponseEntity<ResponseData<DataBody>>(responseData,
				HttpStatus.OK);
		return response;
	}
	 ...
	 * final String sendUrl = "http://message-service/message/sms/send"; HttpHeaders
	 * headers = new HttpHeaders();
	 * headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
	 * MultiValueMap<String, String> map = new LinkedMultiValueMap<String,
	 * String>(); map.add("mobile", mobile); map.add("templateId", "CHECK_CODE");
	 * map.add("params['code']", code); log.info("发送参数：{}", map);
	 * 
	 * HttpEntity<MultiValueMap<String, String>> request = new
	 * HttpEntity<MultiValueMap<String, String>>(map, headers);
	 * ResponseEntity<SmsSendResponse> response =
	 * restTemplate.postForEntity(sendUrl, request, SmsSendResponse.class); return
	 * response; }
	 */
}
